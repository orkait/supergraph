...
def test_prompt_assembly_guidelines_and_skills(tmp_path, monkeypatch):
    ...
    assert 0 < approx_tokens(core_prompt()) < 1100 and approx_tokens(build_system_prompt(PromptInputs(cwd=Path("/nonexistent"), mode=Mode.ASK))) < 1700
    ...
    crowd = [*skills, *(Skill(f"s{i:03d}", "d" * LIMITS.skill_description_chars, "", "p") for i in range(60))]
    block = skills_block(crowd)
    index = "\n".join(line for line in block.splitlines() if line.startswith("- "))
    assert "- bench:" in block and "BODY" not in block and block.count("\n- ") == len(crowd) and len(index) <= LIMITS.skills_index_bytes
    assert skills_block([Skill(f"s{i:04d}", "d" * 200, "", "p") for i in range(400)]).count("\n- ") == 400
