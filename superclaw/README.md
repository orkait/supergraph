...
| Claude Code plugins | `SUPERCLAW_CLAUDE_PLUGINS=1` loads every plugin Claude Code has installed (`~/.claude/plugins/installed_plugins.json`), or `superclaw plugin install --link <dir>` links one checkout | off; a Claude plugin is `.claude-plugin/plugin.json` (`name`, optional `skills`, `agents`, `commands`, `hooks`, `mcpServers` paths) plus `skills/*/SKILL.md`, `agents/*.md`, `hooks/hooks.json` and `.mcp.json`; hooks map `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse` and `Stop` to superclaw's events, run through `/bin/sh -c` with `CLAUDE_PLUGIN_ROOT` set and `${CLAUDE_PLUGIN_ROOT}` substituted, receive Claude's stdin fields (`hook_event_name`, `session_id`, `tool_name`, `tool_input`, `cwd`, `source`) next to superclaw's, and may answer with `additionalContext`, `hookSpecificOutput.additionalContext`, `systemMessage` or `decision: block` with a `reason`. `SessionStart` fires once per session per process with `source` `startup` or `resume`. Other Claude events are ignored |
