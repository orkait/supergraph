...
| Knows the layout | a repo map (counts, important files, languages, paths) can ride the system prompt from turn one, budgeted, so the model reads the right file instead of listing directories first. Off by default since it costs window on every turn: `/config repo_map on` turns it on for the session and the next launch; `superclaw repo-map --query auth tokens` ranks paths the same way |
...
|---|---|---|
| Provider key | `superclaw setup [--provider openrouter\|groq\|cerebras\|ollama\|aistudio\|nvidia_nim\|opencode\|local]`, `/setup` in the TUI, or the provider's env var | saved to `~/.config/superclaw/credentials.env` (mode 600) together with `SUPERCLAW_MODEL`; the environment overrides the file. `local` takes a base URL instead of a key (`LOCAL_LLM_BASE_URL`, e.g. `http://127.0.0.1:8081/v1`) and picks the first model the server lists. The TUI opens without a key and shows the setup screen; `exec` refuses to run without one |
...
| Repo map | `/config repo_map on\|off`, `SUPERCLAW_REPO_MAP`; `superclaw repo-map [--json\|--query TEXT\|--chart]` prints it, `--chart` as bars over languages, top-level directories and extensions | off; a deterministic map of the workspace (`git ls-files` when in a repository, else a walk that skips build and cache directories) rendered as counts, the files that usually matter, languages and paths, clamped to 6,000 bytes and placed in the system prompt as `<repo_map>` so the model knows the layout on turn one. `--query` ranks paths by term |
...
|---|---|
| Renders the answer itself | superclaw owns the markdown pipeline rather than handing the model's text to a widget, because a widget reflows what it does not recognise. `render.py` turns markdown into lines: prose wraps at a measure of 88 while tables and code use the full width, headings carry weight instead of a colour, lists hang their indent, code blocks get a left bar, a language label and real syntax highlighting, and tables are content-fit with one header rule, alignment from `---:`, cells that wrap rather than clip, and row rules only when a cell wrapped. A block that contains box-drawing, block, braille or arrow glyphs, or that holds aligned columns, is preformatted and passes through byte for byte - so a diagram the model draws without a fence survives instead of being reflowed into one paragraph |
| Renders while it streams | the answer is rendered as it arrives, not raw until the end. `render.Stream` keeps the settled prefix (everything up to the last blank line outside an open fence) already rendered and only re-renders the tail, so cost stays flat as the answer grows: 977 updates over a 6.8 KB answer cost 283 ms in total, and the streamed result is byte-identical to rendering the finished document in one pass |
| Draws its own charts | `viz.py` is the chart surface: bars and gauges with eighth-block sub-cell precision, sparklines, column charts, braille line plots at 2x4 dots per cell, shade heatmaps, indented trees, and a layered box-and-arrow flow diagram whose edges are routed by accumulating a direction bitmask per cell and looking the join glyph up at the end. The context gauge in the status bar and `/context` both use it. Everything degrades to ASCII ramps when the locale is not UTF-8 or `SUPERCLAW_ASCII=1` |
| Renders headless too | `superclaw exec` prints a rendered answer when stdout is a terminal and the exact text when it is a pipe, so a shell pipeline still gets what the model wrote |
...
| `/config [key [value]]` | every setting in one editor: model, fallback, effort, mode, sandbox, repo_map, stream, intent_gate, max_turns, budget_tokens, budget_usd, context_window, output_tokens, tui, search, claude_config. Bare opens the list with each value and what it does; enter cycles a toggle or a choice and edits a number or a name; `/config repo_map on` sets one without opening anything. Everything lands in `credentials.env` next to the keys, so the next launch starts where you left off, and the three that cannot change under a running session (`tui`, `search`, `claude_config`) say so in their row. `superclaw config` is the same surface from the shell: bare lists, `superclaw config <key>` reads, `superclaw config <key> <value>` writes |
| `/sandbox [on\|off]` | bare shows the state; `off` unwraps `bash` and the python kernel onto the host, where every command asks first outside unsafe mode, and `on` puts them back inside bubblewrap. It takes effect on the running session and is saved for the next launch, the same setting `--sandbox on\|off` and `SUPERCLAW_SANDBOX` carry. Turning it on without bubblewrap installed fails with the install command rather than pretending |
