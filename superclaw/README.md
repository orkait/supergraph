...
The system prompt is 653 tokens (950 with the confirmation policy). Only six tool schemas ride every request (`read_file` `edit_file` `write_file` `grep` `bash` `tool_search`); the rest are listed one line each and load on demand through `tool_search`, so a first turn is about 2k tokens before the user's message (eager=1000 all=2194 prompt=969 first_turn=1969, measured with the ink-quarter estimator).
