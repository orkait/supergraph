...
| Agent profiles | `--agent NAME`, `/agent` in the TUI | two built in, `explore` and `review`, both read-only; then `<workspace>/.superclaw/agents/<name>.md` then `~/.config/superclaw/agents/<name>.md`, frontmatter `name` `description` `tools` `model` over a prompt body. `superclaw agents` lists them |
