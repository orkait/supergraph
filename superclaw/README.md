...
| Searches the web | `web_search` (deferred, loads through `tool_search`): Google's Custom Search JSON API when `GOOGLE_API_KEY` and `GOOGLE_CSE_ID` are set, DuckDuckGo's HTML endpoint otherwise with no key at all; `limit` up to 10 and an optional `domains` allowlist that drops other hosts before the model sees them; results come back as numbered title, URL and snippet, redacted and budgeted like every tool output. It is a network tool, so `ask` mode prompts once per query and plan mode hides it |
| Reads pages | `web_fetch` (deferred): a public http or https URL, HTML converted to compact markdown (headings, links, lists, code blocks) and everything else returned as text, capped at `max_bytes` (256 KiB by default, 2 MiB at most) and 30 s, five redirects each checked again. Loopback, private, link-local and other special-use hosts are refused even when a public hostname resolves to one, so a fetched page cannot be turned into a probe of this machine; local servers are for `bash` with `curl`, where the network prompt applies. The whole page is stored as an observation, so `recall §id` brings back what the output budget trimmed |
...
| `/doctor` | terminal, sandbox, mode, agent, model, store, MCP, the modern host tools present, and connected-provider health, read-only |
